...
from src.baseline_v4.model import (
    add_ranker_scores,
    fit_ranker_model,
    get_feature_importance_df,
    train_valid_split_by_user,
)
from src.baseline_v4.prepare_data import get_feature_columns, make_pointwise_dataset
from src.common.metrics import evaluate_ranking


def main() -> None:
    data_dir = Path("data/raw/ml-1m")
    report_dir = Path("reports/baseline_v4")
    ...
    scored_valid_df = add_ranker_scores(
        model=model,
        df=valid_df,
        feature_cols=feature_cols,
    )
    ...
    with (report_dir / "metrics_yetirank.json").open("w", encoding="utf-8") as f:
        json.dump(metrics, f, ensure_ascii=False, indent=2)

    fi_df = get_feature_importance_df(model, valid_df, feature_cols, cat_features)
